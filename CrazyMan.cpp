...
// 加载炮弹
void CrazyMan::Archery(int iTime, T_Scene* scene) {
	...
	if (GetEndTime() - GetStartTime() >= (DWORD)iTime)
	{
		...
		if (type == 0) arrow_set.push_back(new T_Sprite(L".\\res\\sprite\\ShooterArrow.png", 320, 128));
		else if (type == 1) arrow_set.push_back(new T_Sprite(L".\\res\\sprite\\FighterWeapon.png", 320, 128));
		else if (type == 2) arrow_set.push_back(new T_Sprite(L".\\res\\sprite\\ElementalSword.png", 320, 128));
...
void CrazyMan::Attack(T_Scene* scene, vSpriteSet npc_set)
{
	vSpriteSet::iterator p;
	for (p = npc_set.begin(); p != npc_set.end(); p++) // 处理与NPC碰撞
	{
		// 玩家触碰到NPC的处理
		if (CollideWith((*p)) && !(*p)->IsDead() && (*p)->IsVisible())
		{
			(*p)->SetVisible(false);
			(*p)->SetActive(false);
			(*p)->SetDead(true);
